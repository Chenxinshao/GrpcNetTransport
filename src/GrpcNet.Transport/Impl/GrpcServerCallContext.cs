namespace GrpcNet.Transport.Impl
{
    ...
    internal class GrpcServerCallContext : ServerCallContext, IDisposable
    {
        ...
        private readonly GrpcTransportConnection _connection;
        ...
        public GrpcServerCallContext(
            string method,
            string host,
            string peer,
            DateTime? deadline,
            Metadata requestHeaders,
            GrpcTransportConnection connection,
            Mutex writeMutex,
        ...
        protected override async Task WriteResponseHeadersAsyncCore(Metadata responseHeaders)
        {
            ...
            using (await _writeMutex.WaitAsync(CancellationTokenSource.Token).ConfigureAwait(false))
            {
                ...
                {
                    Type = GrpcMessageType.ResponseHeaders
                }, CancellationTokenSource.Token).ConfigureAwait(false);
                await _connection.WriteAsync(
                    GrpcMetadataConverter.Convert(responseHeaders),
                    CancellationTokenSource.Token).ConfigureAwait(false);
                _headersSent = true;
            }
        }
