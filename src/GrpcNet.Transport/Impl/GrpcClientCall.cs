namespace GrpcNet.Transport.Impl
{
        ...
        where TResponse : class
    {
        ...
        private GrpcTransportConnection? _connection;
        ...
        private void LogTrace(string message)
        {
            if (_logger != null)
            {
                _logger.LogTrace($"GrpcAsyncUnaryCall: {message}");
            }
        }
        ...
        private async Task ExecuteCallAsync()
        {
            ...
            try
            {
                await using ((await GrpcTransportConnection.ConnectAsync(_endpoint, _transportFactory, _logger, cts.Token).ConfigureAwait(false))
                    ...
                    .ConfigureAwait(false))
                {
                    try
                    {
                        ...
                        {
                            await _connection.WriteAsync(new GrpcRequest
                            {
                                FullName = _method.FullName,
                                HasRequestHeaders = _options.Headers != null,
                                RequestHeaders = GrpcMetadataConverter.Convert(_options.Headers),
                        ...
                        if (!IsStreamingRequestsToServer)
                        {
                            ...
                            using (await _writeMutex.WaitAsync(cts.Token).ConfigureAwait(false))
                            {
                                await _connection.WriteAsync(new GrpcMessage
                                {
                                    Type = GrpcMessageType.RequestData,
                                }, cts.Token).ConfigureAwait(false);
                        ...
                        while (true)
                        {
                            ...
                            try
                            {
                                message = await connection.ReadExpectedAsync<GrpcMessage>(GrpcMessage.Descriptor, cts.Token).ConfigureAwait(false);
                            }
                            ...
                            switch (message.Type)
                            {
                                case GrpcMessageType.ResponseHeaders when _responseHeaders == null && _responseData == null && _responseStatus == null:
                                    {
                                        LogTrace($"Reading response headers from server.");
                                        var headers = await connection.ReadExpectedAsync<GrpcMetadata>(GrpcMetadata.Descriptor, cts.Token).ConfigureAwait(false);
                                        _responseHeaders = GrpcMetadataConverter.Convert(headers);
                                        _responseHeadersReady.Release();
                                        break;
                                    }
                                case GrpcMessageType.ResponseData when _responseData == null || IsStreamingResponsesFromServer:
                                    {
                                        LogTrace($"Reading response data from server.");
                                        using (var memory = await connection.ReadBlobAsync(cts.Token).ConfigureAwait(false))
                                        {
                                            var nextItem = _method.ResponseMarshaller.ContextualDeserializer(new GrpcDeserializationContext(memory.Memory));
                                ...
                                case GrpcMessageType.ResponseComplete:
                                    {
                                        LogTrace($"Reading response completion from server.");
                                        var complete = await connection.ReadExpectedAsync<GrpcResponseComplete>(GrpcResponseComplete.Descriptor, cts.Token).ConfigureAwait(false);
                                        _responseStatus = new Status((StatusCode)complete.StatusCode, complete.StatusDetails);
                                        if (complete.HasResponseTrailers)
                                        {
                                            _responseTrailers = GrpcMetadataConverter.Convert(complete.ResponseTrailers);
                                        }
                                        _responseHeadersReady.Release();
                                        _responseReady.Release();
                    ...
                    using (await _writeMutex.WaitAsync(_cancellationCts.Token).ConfigureAwait(false))
                    {
                        ...
                        if (!_connection.HasWriteBeenInterrupted)
                        {
                            ...
                            try
                            {
                                await _connection.WriteAsync(new GrpcMessage
                                {
                                    Type = GrpcMessageType.RequestCancel,
        ...
        async Task IClientStreamWriter<TRequest>.CompleteAsync()
        {
            ...
            if (IsStreamingRequestsToServer)
            {
                ...
                using (await _writeMutex.WaitAsync(_cancellationToken).ConfigureAwait(false))
                {
                    await _connection.WriteAsync(new GrpcMessage
                    {
                        Type = GrpcMessageType.RequestComplete,
